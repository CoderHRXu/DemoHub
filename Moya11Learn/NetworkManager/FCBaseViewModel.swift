...
class FCBaseViewModel {
    ...
    func sendRequest<T:TargetType>(target : T , withHUD hasHUD: Bool = false ) -> Observable<Response> {
        ...
        let myEndPointClosure = { (target : T) -> Endpoint<T> in
            
            let url = target.baseURL.appendingPathComponent(target.path).absoluteString
            
            let endpoint = Endpoint<T>.init(url: url, sampleResponseClosure:{.networkResponse(200, target.sampleData) }, method: target.method, task: target.task, httpHeaderFields: target.headers)
