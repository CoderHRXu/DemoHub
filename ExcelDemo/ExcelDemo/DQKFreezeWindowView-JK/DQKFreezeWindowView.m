...
- (void)reloadFrame:(CGRect)frame FreezePoint: (CGPoint) freezePoint cellViewSize: (CGSize) cellViewSize {
    
    _mainScrollView.frame       = CGRectMake(freezePoint.x, freezePoint.y, frame.size.width - freezePoint.x, frame.size.height - freezePoint.y);
    _sectionScrollView.frame    = CGRectMake(freezePoint.x, 0, frame.size.width - freezePoint.x, freezePoint.y);
    _rowScrollView.frame        = CGRectMake(0, freezePoint.y, freezePoint.x, frame.size.height - freezePoint.y);
    _signView.frame             = CGRectMake(0, 0, freezePoint.x, freezePoint.y);
    [self reloadData];
}

- (void)setSignViewWithContent:(NSString *)content SecondContent:(NSString *)seContent DQKSignViewStyle:(DQKSignViewStyle)style {
    [self.signView setContent:content SecondContent:seContent DQKSignViewStyle:style];
}
...
- (void)reloadData {
    ...
    [_sectionScrollView.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
    [_rowScrollView.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
//    [_signView.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
    [self reloadViews];
}
