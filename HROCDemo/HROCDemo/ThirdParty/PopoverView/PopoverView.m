...
float PopoverDegreesToRadians(float angle) {
...
- (void)showToPoint:(CGPoint)toPoint {
    ...
    [maskPath addArcWithCenter:CGPointMake(cornerRadius, cornerRadius + maskTop)
                        radius:cornerRadius
                    startAngle:PopoverDegreesToRadians(180)
                      endAngle:PopoverDegreesToRadians(270)
                     clockwise:YES];
    // 箭头向上时的箭头位置
    if (_isUpward) {
    ...
    [maskPath addArcWithCenter:CGPointMake(currentW - cornerRadius, maskTop + cornerRadius)
                        radius:cornerRadius
                    startAngle:PopoverDegreesToRadians(270)
                      endAngle:PopoverDegreesToRadians(0)
                     clockwise:YES];
    // 右下圆角
    ...
    [maskPath addArcWithCenter:CGPointMake(currentW - cornerRadius, maskBottom - cornerRadius)
                        radius:cornerRadius
                    startAngle:PopoverDegreesToRadians(0)
                      endAngle:PopoverDegreesToRadians(90)
                     clockwise:YES];
    // 箭头向下时的箭头位置
    if (!_isUpward) {
    ...
    [maskPath addArcWithCenter:CGPointMake(cornerRadius, maskBottom - cornerRadius)
                        radius:cornerRadius
                    startAngle:PopoverDegreesToRadians(90)
                      endAngle:PopoverDegreesToRadians(180)
                     clockwise:YES];
    [maskPath closePath];
    // 截取圆角和箭头
