...
#define kButtomImageViewHeight 0
...
#define kTextSelectColor [UIColor colorWithRed:51/255.0 green:142/255.0 blue:255/255.0 alpha:1]
...
- (void)selectIndexPath:(DOPIndexPath *)indexPath triggerDelegate:(BOOL)trigger {
    ...
    NSLog(@"%ld", (long)[_dataSource menu:self numberOfItemsInRow:indexPath.row column:indexPath.column]);
    ...
    if (indexPath.item < 0 ) {
        if (!_isClickHaveItemValid && [_dataSource menu:self numberOfItemsInRow:indexPath.row column:indexPath.column] > 0){
            ...
            title.string = [_dataSource menu:self titleForRowAtIndexPath:[DOPIndexPath indexPathWithCol:indexPath.column row:self.isRemainMenuTitle ? 0 : indexPath.row]];
            if (trigger) {
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        //righttableView init
        _rightTableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x + self.frame.size.width/3, self.frame.origin.y + self.frame.size.height, self.frame.size.width/3 *2, 0) style:UITableViewStylePlain];
        ...
        _rightTableView.backgroundColor = [UIColor colorWithRed:244/255.0 green:244/255.0 blue:244/255.0 alpha:1.0];
...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (show) {
        if (haveItems) {
            ...
            _rightTableView.frame = CGRectMake(self.origin.x + self.frame.size.width/3, self.frame.origin.y + self.frame.size.height, self.frame.size.width/3 *2, 0);
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
                ...
                _rightTableView.frame = CGRectMake(self.origin.x + self.frame.size.width/3, self.frame.origin.y + self.frame.size.height, self.frame.size.width/3 *2, tableViewHeight);
            } else {
...
//        bg.backgroundColor = [UIColor whiteColor];
        bg.backgroundColor = [UIColor colorWithRed:244/255.0 green:244/255.0 blue:244/255.0 alpha:1.0];
...
//            cell.accessoryView = nil;
            cell.accessoryView = [[UIImageView alloc]initWithImage:nil highlightedImage:[UIImage imageNamed:@"optionSelected"]];
        }
